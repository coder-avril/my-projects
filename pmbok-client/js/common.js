/********************* 常量 ***********************/
const REAL_BASE_URL = "http://localhost:8080/pmbok/";
const USER = 'user';
const TOKEN = 'token';
...
class Commons {
    ...
    static _addTokenHeader(cfg) {
        // 取出token
        const token = Datas.get(USER, TOKEN)
        if (token) {
            if (!cfg.headers) {
                cfg.headers = {}
            }
            // 将token放到请求头
            cfg.headers[TOKEN_HEADER] = token
        }
    }
}
